...
namespace DataTableAccessLayer
{
	...
	public class SqlDataTableAccessLayer
	{
		...
		public void RestoreDataSetFromBackup(DataSet dataSet)
		{
			...
			if (File.Exists(dataSet.DataSetName + ".xml"))
				dataSet.ReadXml(dataSet.DataSetName + ".xml", XmlReadMode.ReadSchema);
			else
				Console.WriteLine("Create backup first!!");

			for (int i = 0; i < dataSet.Tables.Count; i++)
			{
				DataTable table = dataSet.Tables[i];
				Debug.WriteLine("Adding rows from Table " + table.TableName );
		...
		/// <summary>
		/// Added by Steven to insert records in database tables on restore 
		/// </summary>
		/// <param name="table"></param>
		public void BulkInsert(DataTable table) 
		{
			...
			Debug.WriteLine("ClearDatabaseTable: " + bulkInsertCommand);

			using (SqlCommand sqlCommand = new SqlCommand(bulkInsertCommand, sqlConnection))
			{
				sqlCommand.ExecuteNonQuery();
			}*/
